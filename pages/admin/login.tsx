...
const Login: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (username === adminUsername && password === adminPassword) {
      localStorage.setItem('gamaAdminAuth', 'true');
      router.push('/admin');
    } else {
  ...
  return (
    <>
      <Head>
        <title>Admin Login - Gama Bina Güçlendirme</title>
        <meta name="description" content="Admin login for Gama Bina Güçlendirme" />
      </Head>
        ...
        <div className="max-w-md w-full space-y-8">
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <button
                type="submit"
                className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-primary-blue hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-blue"
              >
                Sign in
              </button>
            </div>
